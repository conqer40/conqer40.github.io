...
export function ResumePage({ lang }) {
  const c = d[lang];
  return <main className="resume-page"><section className="resume-hero"><div><span>{c.eyebrow}</span><h1>{c.title}</h1><p>{c.intro}</p><div><a href="/Mohamed-Elhawy-CV.pdf" download><FiDownload />{c.download}</a><a href="mailto:m.elhawy2023@gmail.com"><FiMail />{lang === "ar" ? "تواصل معي" : "Contact me"}</a></div></div><img src="/assets/mohamed-elhawy-work.jpg" alt="Mohamed Elhawy at work" /></section><section className="resume-section"><header><span>01</span><h2>{c.experience}</h2></header><div className="resume-jobs">{c.jobs.map(([date,title,company,items]) => <article key={title}><time>{date}</time><h3>{title}</h3><p>{company}</p><ul>{items.map(x => <li key={x}><FiCheckCircle />{x}</li>)}</ul></article>)}</div></section><section className="resume-section split"><div><header><span>02</span><h2>{c.skills}</h2></header><div className="skill-list">{c.skillList.map(x => <span key={x}>{x}</span>)}</div></div><div><header><span>03</span><h2>{c.education}</h2></header><ul className="education-list">{c.edu.map(x => <li key={x}>{x}</li>)}</ul></div></section><section className="resume-cta"><h2>{lang === "ar" ? "هل تريد مناقشة فرصة أو مشروع؟" : "Want to discuss an opportunity or project?"}</h2><a href="mailto:m.elhawy2023@gmail.com">m.elhawy2023@gmail.com <FiExternalLink /></a></section></main>;
}
