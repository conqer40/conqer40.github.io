...
export function PersonalHome({ lang }) {
  ...
  return (
    <main className="eh-home">
      ...
      <section className="editorial-section home-resources">
        <HomeSectionIntro kicker={lang === "ar" ? "كورسات ومصادر" : "COURSES & RESOURCES"} title={lang === "ar" ? "مصادر موثوقة" : "Trusted resources"} text={lang === "ar" ? "مقالات تعليمية وملفات وقوالب وأدلة عملية مرتبة في تصنيفات واضحة." : "Organized learning articles, files, templates and guides."} to="/articles/category/kwrsat" link={lang === "ar" ? "استكشف الكورسات" : "Explore courses"} />
        <HomeContentGrid className="resource-strip" loading={homeData.loading} empty={lang === "ar" ? "لا توجد ملفات منشورة حاليًا" : "No published resources yet"}>{homeData.library.map(item => <Resource key={item.id} item={item} />)}</HomeContentGrid>
      </section>
